...
(() => {
  ...
  function setupForm() {
    ...
    // FAVN : afficher/masquer la section détail
    ...
    if (dest.titer_test && favcBloqueante) {
      ...
      if (d.favn.tiene) {
        if (d.favn.resultado === 'fallido') {
          ...
          ejeC.issues.push({
            severity: 'red',
            msg: `Test FAVN/sérologique échoué (résultat < 0,5 UI/mL). Une revaccination et un nouveau prélèvement sont nécessaires. Contacter Zoovet Travel pour un nouveau protocole.`,
            topic: 'favn-fallido'
          });
          ...
          ejeC.issues.push({
            severity: 'red',
            msg: `La date de prélèvement FAVN (${fmtDate(d.favn.fecha_muestra)}) est antérieure à la pose de la micropuce (${fmtDate(d.microchip.fecha)}). Le test est invalide sans puce préalable. Vérifier les dates.`,
          ...
          const infoMsg = renovadaDespues
            ? `Test FAVN/sérologique approuvé (≥0,5 UI/mL). Vaccination renouvelée le ${fmtDate(d.vacuna.fecha)}, après le prélèvement. Valide tant que la vaccination reste à jour.`
            : `Test FAVN/sérologique approuvé (≥0,5 UI/mL)${d.favn.fecha_muestra ? '. Date de prélèvement : ' + fmtDate(d.favn.fecha_muestra) : ''}.`;
  ...
  function buildPDFContent(d, r, destinoLabel, col, fmtU) {
    ...
    if (favsRequired) {
      const favStatusMsg = favsDeclaredOk
        ? 'TITRE: ≥0,5 UI/ML'
        : (favsBlocked ? 'ÉCHANTILLON REQUIS EN URGENCE' : 'EN ATTENTE — NON DÉCLARÉ');
      addRow('TEST TITRATION FAVN RNATT', favsDeclaredOk,
        favStatusMsg,
    ...
    if (favsRequired) {
      nodes.push({ label: 'TEST FAVN', date: d.favn.fecha_muestra ? fmtU(d.favn.fecha_muestra) : '—', status: favsDeclaredOk ? 'cleared' : 'pending' });
    }
