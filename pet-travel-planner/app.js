...
(() => {
  ...
  function computeSemaforo(d) {
    ...
    if (dest.titer_test && dest.titer_test.es_bloqueante) {
      ...
      if (diasAlViaje < diasEsperaFAVN) {
        ...
        ejeC.issues.push({
          severity: 'yellow',
          msg: `Tight FAVN deadline. Collect sample TODAY (${fmtDate(today())}) — sample must be sent, results returned and endorsed.`,
          topic: 'favn-justo'
        });
      }
    }
  ...
  function renderResult(d, r) {
    ...
    html += `<p class="text-xs text-[#1a2e35]/50 text-center mt-6 leading-relaxed">Result generated by Planner v${VERSION} · ${fmtDate(today())}. Informational only. <a href="./methodology.html" class="underline">View methodology</a> · Always verify with official authority before traveling.</p>`;
